import type { ChaseSummary } from "@/lib/utils";
...
export function ChaseSummaryPanel({
  ...
  return (
    <div
      className={
        className ??
        "min-w-[11rem] rounded-md border border-[var(--color-line)] bg-[var(--color-sand)] px-3 py-2.5"
      }
      role="status"
      aria-label={
    ...
    >
      ...
      <p className="mt-0.5 text-2xl font-semibold tabular-nums text-[var(--color-skyline-deep)]">
        {chase.target}
      </p>
      {chase.targetAchieved ? (
